...
The explicit schema-4 profile admits standalone Collection, Configurator and
Editorial Workflow artifacts. Its kind and validated id become part of the
navigation profile, so another artifact cannot enter through an otherwise
identical feature set. Every route receives the structural CSS for that typed
profile, while only an active route embeds its verified source. The browser
validates the complete route plan, contract revision, UTF-8 length and SHA-256
before replacement, then appends the package-owned readiness suffix for that
exact kind. Each Configurator or Editorial Workflow destination starts from
its delivered HTML and creates fresh state through its existing package
