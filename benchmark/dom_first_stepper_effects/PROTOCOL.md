# DOM-first Stepper Effects Benchmark Protocol

This protocol was frozen before the first four-cell build and measurement.
...
Every cell carries equivalent content, CSS, fonts and stepper behaviour. An
accepted Next action selects the second panel and focuses that panel only:

1. hand-written DOM as the practical floor;
2. statically rendered Next.js as the competitive reference;
3. esen_seo DOM-first with the compiled `stepper-effects` transition;
4. the existing visible shell followed by a Flutter release build.

The complete three-step document must remain readable without JavaScript in
the first three cells. The DOM-first network log must contain no Flutter
artifact.
