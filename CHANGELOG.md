...
* Added Carousel as the sixth explicit typed application-runtime handoff
  family. Static and active routes share schema 4, stable Carousel CSS and one
  verified standalone artifact while every destination reconstructs slide
  count and initial selection from its delivered document.
...
* Added route-scoped application runtime bundles. Two or three bundle-capable
  ...
  `SeoDomFirstApplicationRuntime.bundle`. Bundle configuration is bounded,
  strictly shaped JSON; member order is canonical, Stepper and Stepper Effects
  remain one ownership family, and Collection stays on its standalone runtime
  after every measured Collection pair exceeded the unchanged artifact budget.
  Schema-2 manifests bind the complete member set and collision-free filenames
  distinguish equal bundle ids with different members. Every source keeps the
  existing pure-Dart graph and held-state checks, the complete bundle fails
  closed when one member is invalid, and existing single-runtime schema and
  generated bytes remain unchanged. The fixed 25 KiB gzip and 512 KiB raw
  artifact ceilings also apply to bundles. Encoded path separators and all Dart
  analyzer keywords are rejected before file access or compilation.
  `SeoDomFirstApplicationRuntime` now has a bundle subtype, so applications with
  exhaustive switches over that sealed hierarchy must handle the new case.
