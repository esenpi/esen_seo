...
* Added route-scoped application runtime bundles. Two to four different
  ...
  remain one ownership family, and schema-2 manifests bind the complete member
  set. Every source keeps the existing pure-Dart graph and held-state checks,
  the complete bundle fails closed when one member is invalid, and the existing
  single-runtime schema and generated bytes remain unchanged. The fixed 25 KiB
  gzip and 512 KiB raw artifact ceilings also apply to bundles.
