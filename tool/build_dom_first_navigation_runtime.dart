...
Future<void> main(List<String> arguments) async {
  ...
  if (arguments.any((argument) => argument != '--write')) {
    stderr.writeln(
      'Usage: dart run tool/build_dom_first_navigation_runtime.dart [--write]',
    );
    exitCode = 64;
    return;
  }
...
let plain=v=>typeof v==="string"&&v.length<=256&&!/[\u0000-\u001f\u007f\u061c\u200e\u200f\u202a-\u202e\u2066-\u2069]/.test(v),norm=p=>{if(typeof p!=="string"||!p.startsWith("/"))return null;while(p.length>1&&p.endsWith("/"))p=p.slice(0,-1);return p},segments=p=>{p=norm(p);if(p==null)return null;let out=[];for(let s of p.split("/"))try{out.push(decodeURIComponent(s))}catch(_){return null}return out},parseManifest=doc=>{
 ...
 if(node.parentElement!==doc.head||node.type!=="application/json"||new TextEncoder().encode(raw).byteLength>MAN||names.some(n=>!allowed.has(n)))return null;try{j=JSON.parse(raw)}catch(_){return null}
 if(!j||Array.isArray(j)||Object.keys(j).sort().join(",")!=="base,profile,routes,schema"||j.schema!==1||!plain(j.base)||!plain(j.profile)||!Array.isArray(j.routes)||j.routes.length<1||j.routes.length>256)return null;
 let base=norm(j.base);if(base==null||node.dataset.esenNavigationProfile!==j.profile)return null;for(let r of j.routes){if(!Array.isArray(r)||r.length!=2||!plain(r[0])||norm(r[0])==null||(r[1]!==null&&!plain(r[1])))return null}
 return{node,base,profile:j.profile,routes:j.routes}
},pagePath=p=>{let parts=segments(p),s=parts&&parts[parts.length-1].toLowerCase(),dot=s&&s.lastIndexOf(".");return !s||dot<0?true:dot===0||dot===s.length-1?false:pageExt.has(s.slice(dot+1))||/^v?\d+(?:\.\d+)+(?:[-+][a-z0-9.-]+)?$/i.test(s)},routePath=(pathname,base)=>{let actual=segments(pathname),normalizedBase=norm(base),prefix=normalizedBase&&normalizedBase.split("/");if(!actual||!prefix)return null;if(prefix.length==2&&prefix[1]==="")prefix=[""];if(prefix.length>1){if(actual.length<prefix.length)return null;for(let i=1;i<prefix.length;i++)if(actual[i]!==prefix[i])return null;let rest=actual.slice(prefix.length);actual=rest.length?["",...rest]:["",""]}return actual},matches=(actual,pattern)=>{pattern=norm(pattern);if(pattern==null)return false;let parts=pattern.split("/");if(parts.length!==actual.length)return false;for(let i=0;i<parts.length;i++)if(parts[i].startsWith(":")){if(!actual[i])return false}else{let decoded;try{decoded=decodeURIComponent(parts[i])}catch(_){return false}if(decoded!==actual[i])return false}return true},profileFor=(url,plan)=>{if(!pagePath(url.pathname))return null;let actual=routePath(url.pathname,plan.base);if(!actual)return null;for(let r of plan.routes)if(matches(actual,r[0]))return r[1];return null},validUrl=(url,plan)=>url.origin===location.origin&&!url.username&&!url.password&&(url.protocol==="http:"||url.protocol==="https:")&&profileFor(url,plan)===plan.profile,samePlan=(a,b)=>a.base===b.base&&a.profile===b.profile&&a.routes.length===b.routes.length&&a.routes.every((r,i)=>r[0]===b.routes[i][0]&&r[1]===b.routes[i][1]),
safeUrl=(name,value)=>{if(/[\u0000-\u001f\u007f]/.test(value))return false;let values=urlLists.has(name)?value.split(/[,\s]+/).filter(v=>v&&!/^[0-9.]+[xw]$/.test(v)):[value];return values.every(v=>{let m=v.trim().match(/^([a-z][a-z0-9+.-]*):/i);return !m||schemes.has(m[1].toLowerCase())})},validContent=root=>{let nodes=root.querySelectorAll("*");if(nodes.length>ELEMENTS)return false;for(let node of nodes){if(!tags.has(node.localName)||node.attributes.length>ATTRS)return false;for(let attr of node.attributes){let name=attr.name;if(name.startsWith("on")||behaviour.has(name)||name.startsWith("data-esen-seo-navigation-")||urlAttrs.has(name)&&!safeUrl(name,attr.value))return false}}return true},
validHead=node=>{let tag=node.localName,names=Array.from(node.attributes).map(a=>a.name),only=allowed=>names.every(n=>allowed.includes(n));if(tag==="title")return only(["data-esen-seo-navigation-head"])&&node.textContent.length<=512;if(tag==="meta")return only(["data-esen-seo-navigation-head","name","property","content"])&&node.hasAttribute("content")&&((node.hasAttribute("name")?1:0)+(node.hasAttribute("property")?1:0)===1)&&plain(node.getAttribute("name")||node.getAttribute("property"));if(tag==="link"){let rel=node.getAttribute("rel"),href=node.getAttribute("href")||"";return only(["data-esen-seo-navigation-head","rel","href","hreflang"])&&(rel==="canonical"||rel==="alternate")&&!!href&&safeUrl("href",href)&&(rel!=="alternate"||plain(node.getAttribute("hreflang")))}if(tag==="script"){if(!only(["data-esen-seo-navigation-head","type"])||node.type!=="application/ld+json"||node.textContent.length>65536)return false;try{JSON.parse(node.textContent);return true}catch(_){return false}}return false},validate=(doc,url,expected)=>{
...
let bounded=n=>Number.isFinite(n)?Math.max(0,Math.min(10000000,Math.round(n))):0,save=()=>{try{history.replaceState({[stateKey]:1,x:bounded(scrollX),y:bounded(scrollY)},"",location.href)}catch(_){}},state=v=>v&&v[stateKey]===1&&Number.isFinite(v.x)&&Number.isFinite(v.y)?{x:bounded(v.x),y:bounded(v.y)}:null,hard=(url,pop)=>{delete d.documentElement.dataset.esenNavigationPending;if(pop)location.reload();else location.assign(url.href)},focusAndScroll=(url,position)=>{let target=null;if(url.hash)try{target=d.getElementById(decodeURIComponent(url.hash.slice(1)))}catch(_){}if(target&&!c.contains(target))target=null;if(!target)target=c.querySelector("main h1,h1");if(target){let natural=target.matches('a[href],button,input,select,textarea,[tabindex]');if(!natural){target.setAttribute("tabindex","-1");target.setAttribute("data-esen-navigation-focus","")}try{target.focus({preventScroll:true})}catch(_){}}if(position)scrollTo(position.x,position.y);else if(url.hash&&target)target.scrollIntoView();else scrollTo(0,0)},apply=(target,url,push,position)=>{
...
},go=async(url,push,position)=>{let token=++g;if(controller)controller.abort();controller=new AbortController;pending=c;pending.setAttribute("aria-busy","true");d.documentElement.dataset.esenNavigationPending="1";try{
...
try{history.scrollRestoration="manual"}catch(_){}if(!state(history.state))history.replaceState({[stateKey]:1,x:bounded(scrollX),y:bounded(scrollY)},"",location.href);
...
String _withApplicationProfileHandoff(String source) {
  ...
  source = _replaceOnce(
    ...
    r'''(x[1]!=="collection"&&x[1]!=="configurator"&&x[1]!=="editorial-workflow"&&x[1]!=="approval-checklist"&&x[1]!=="tabs"&&x[1]!=="carousel")||typeof x[2]''',
  );
  source = _replaceOnce(
    source,
